...
def get_camera(dataroot,sample_data, frame_id):
    jpg_filename=os.path.join(dataroot, sample_data['filename'])
    msg = CompressedImage()
    msg.header.frame_id = frame_id
    msg.header.stamp = get_time(sample_data)
    msg.format = "jpeg"
...
def get_lidar(dataroot,sample_data, frame_id,input_channels=4):
    pc_filename=os.path.join(dataroot, sample_data['filename'])

    suffix = pc_filename.split('.')[-1]
    if suffix == 'bin':
        if input_channels == 4:
            pc = pypcd.PointCloud.from_bin(pc_filename, format="xyzi")
    elif suffix == 'pcd':
        pc = pypcd.PointCloud.from_path(pc_filename)
    else:
        raise Exception('Error: Invalid point cloud file format: %s' % suffix)
...
def get_tfs(nusc,sample,lidar_channel="LIDAR_TOP"):
    sample_lidar = nusc.get('sample_data', sample['data'][lidar_channel])
...
def get_tfmessage(nusc,sample,lidar_channel):
    # get transforms for the current sample
    tf_array = TFMessage()
    tf_array.transforms = get_tfs(nusc,sample,lidar_channel)
    ...
    if next_sample is not None:
        tf_array.transforms += get_tfs(nusc,next_sample,lidar_channel)
...
class Nuscenes2Bag:
    def __init__(self,scene,version,dataroot,lidar_channel="LIDAR_TOP",use_map=False,use_can=False,):
        ...
        self.nusc = NuScenes(version=self.NUSCENES_VERSION, dataroot=self.dataroot, verbose=True)
    ...
    def convert_scene(self,scene):
        ...
        stamp = get_time(self.nusc.get('ego_pose', self.nusc.get('sample_data', cur_sample['data'][self.lidar_channel])['ego_pose_token']))
        ...
        while cur_sample is not None:
            sample_lidar = self.nusc.get('sample_data', cur_sample['data'][self.lidar_channel])
            ...
            for (sensor_id, sample_token) in cur_sample['data'].items():
                ...
                if sample_data['sensor_modality'] == 'camera':
                    msg = get_lidar_imagemarkers(self.nusc,sample_lidar, sample_data, sensor_id)
                    bag.write(topic + '/image_markers_lidar', msg, stamp)
                    write_boxes_imagemarkers(self.nusc,bag, cur_sample['anns'], sample_data, sensor_id, topic, stamp)
            ...
            for (sensor_id, sample_token) in cur_sample['data'].items():
                ...
                while next_sample_token != '':
                    ...
                    if next_sample_data['sensor_modality'] == 'lidar':
                        ...
                        closest_lidar = find_closest_lidar(cur_sample['data'][self.lidar_channel], camera_stamp_nsec)
                        if closest_lidar is not None:
                            ...
                            msg = get_remove_imagemarkers(sensor_id, self.lidar_channel, msg.header.stamp)
                            non_keyframe_sensor_msgs.append((msg.header.stamp.to_nsec(), topic + '/image_markers_lidar', msg))
